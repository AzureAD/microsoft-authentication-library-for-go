...
func TestRealKeyGuardCrossProcessCreationConverges(t *testing.T) {
	...
	for i := 0; i < racers; i++ {
		...
		go func() {
			defer wg.Done()
			// #nosec G204,G702 -- the command is this test binary and the arguments
...
func TestRealKeyGuardPersistedCertificateIsUsable(t *testing.T) {
	...
	if publishTo != "" {
		...
		// #nosec G703 -- publishTo is a test-only path supplied by the interop harness.
...
func TestRealKeyGuardContainerIsStable(t *testing.T) {
	...
	if path := os.Getenv(realKeyGuardPubOut); path != "" {
		...
		// #nosec G703 -- path is a test-only output selected by the interop harness.
