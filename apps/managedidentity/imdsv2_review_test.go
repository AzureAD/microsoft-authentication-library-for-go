...
func TestCustomMtlsClientGetsARedirectRefusalWithoutBeingMutated(t *testing.T) {
	...
	if got == stated {
		t.Fatal("the caller's client was used directly instead of being copied")
	}
	if err := got.CheckRedirect(mustRequest(t, "https://elsewhere.example/"), nil); err != nil {
		t.Fatalf("the caller's explicit redirect policy was replaced: %v", err)
	}
	got.Timeout = time.Minute
	if stated.Timeout != 0 {
		t.Fatal("mutating the copy changed the caller's client")
	}
}
...
// MSAL .NET fixes the retry budget from the first answer but chooses each delay
// from the current one.
func TestRetryBudgetIsPinnedButDelayUsesTheCurrentAnswer(t *testing.T) {
	for _, tc := range []struct {
		name      string
		statuses  []int
		...
		{
			name:      "410 then 500 keeps the long budget but uses exponential delay",
			statuses:  []int{http.StatusGone, http.StatusInternalServerError, http.StatusOK},
			wantWaits: []time.Duration{imdsGoneRetryAfter, imdsRetryDelay(1)},
		},
		{
			name:      "500 then 410 keeps the short budget but uses the Gone delay",
			statuses:  []int{http.StatusInternalServerError, http.StatusGone, http.StatusOK},
			wantWaits: []time.Duration{imdsRetryDelay(0), imdsGoneRetryAfter},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
