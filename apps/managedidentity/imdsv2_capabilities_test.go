...
// IMDSv1 acquisition deliberately retains its shipped fixed endpoint even when
// the pod-identity override redirects IMDSv2. Discovery must probe the endpoint
// acquisition will actually use, or a v1-only sidecar can make discovery
// succeed before acquisition contacts a different service.
func TestV1ProbeUsesTheSameFixedEndpointAsV1Acquisition(t *testing.T) {
	t.Setenv(azurePodIdentityAuthorityHostEnvVar, "http://pod-identity.example")

	var probeScheme, probeHost, probePath string
	v := imdsV2{
		httpClient: stubHTTPClient(func(req *http.Request) (*http.Response, error) {
			probeScheme = req.URL.Scheme
			probeHost = req.URL.Host
			probePath = req.URL.Path
	...
	if err := v.probeV1Endpoint(context.Background(), newCorrelationID()); err != nil {
		t.Fatalf("probeV1Endpoint: %v", err)
	}

	tokenRequest, err := createIMDSAuthRequest(
		context.Background(), SystemAssigned(), "https://vault.azure.net")
	if err != nil {
		t.Fatalf("createIMDSAuthRequest: %v", err)
	}
	if probeScheme != tokenRequest.URL.Scheme ||
		probeHost != tokenRequest.URL.Host ||
		probePath != tokenRequest.URL.Path {
		t.Fatalf(
			"probe endpoint = %s://%s%s, acquisition endpoint = %s://%s%s",
			probeScheme, probeHost, probePath,
			tokenRequest.URL.Scheme, tokenRequest.URL.Host, tokenRequest.URL.Path,
		)
	}
}

// Once the fixed endpoint establishes IMDSv1, the compute document used to
// determine its binding tier must come from that same origin. Consulting the
// pod-identity override would combine the source answer from one service with
// the capability answer from another.
func TestV1ComputeMetadataUsesTheSameFixedOriginAsV1Acquisition(t *testing.T) {
	t.Setenv(azurePodIdentityAuthorityHostEnvVar, "http://pod-identity.example")

	var computeScheme, computeHost, computePath string
	v := imdsV2{
		httpClient: stubHTTPClient(func(req *http.Request) (*http.Response, error) {
			computeScheme = req.URL.Scheme
			computeHost = req.URL.Host
			computePath = req.URL.Path
			return &http.Response{
				StatusCode: http.StatusOK,
				Header:     make(http.Header),
				Body: io.NopCloser(strings.NewReader(
					`{"osType":"Windows","securityProfile":{"securityType":"TrustedLaunch"}}`)),
				Request: req,
			}, nil
		}),
		miType:       SystemAssigned(),
	...
	if _, err := v.getComputeMetadata(context.Background(), newCorrelationID()); err != nil {
		t.Fatalf("getComputeMetadata: %v", err)
	}
	...
	if computeScheme != tokenRequest.URL.Scheme ||
		computeHost != tokenRequest.URL.Host ||
		computePath != imdsComputePath {
		t.Fatalf(
			"compute endpoint = %s://%s%s, want fixed IMDS origin %s://%s%s",
			computeScheme, computeHost, computePath,
			tokenRequest.URL.Scheme, tokenRequest.URL.Host, imdsComputePath,
		)
	}
}
