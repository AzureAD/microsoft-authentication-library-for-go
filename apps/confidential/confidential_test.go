...
func fakeClientNoInstanceDiscovery(tk accesstokens.TokenResponse, credential Credential, options ...Option) (Client, error) {
	options = append(options, WithAuthority("https://fake_authority/fake"))
	options = append(options, WithInstanceDiscovery(false))
	client, err := New("fake_client_id", credential, options...)
	if err != nil {
		return Client{}, err
	}
...
func TestAcquireTokenByAuthCodeNoInstanceDiscovery(t *testing.T) {
	cred, err := NewCredFromSecret("fake_secret")
	if err != nil {
		t.Fatal(err)
	}
	client, err := fakeClientNoInstanceDiscovery(accesstokens.TokenResponse{
		AccessToken:   token,
		RefreshToken:  refresh,
		ExpiresOn:     internalTime.DurationTime{T: time.Now().Add(1 * time.Hour)},
		ExtExpiresOn:  internalTime.DurationTime{T: time.Now().Add(1 * time.Hour)},
...
func TestNewCredFromCertChainNoInstanceDiscovery(t *testing.T) {
	for _, file := range []struct {
		path     string
		numCerts int
	}{
