...
func explain(exchangeScope string) {
	...
	fmt.Println()
	fmt.Println("  BindingCertificate may be nil, in which case MSAL falls back to the client's own")
	fmt.Println("  certificate credential (see resolveMtlsBindingCert in apps/confidential).")
	fmt.Println()
